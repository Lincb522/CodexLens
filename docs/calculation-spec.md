...
数据取自 Tibo 的公开动态时间线、公开动态聚合源和独立公共预测接口。动态正文经过同一套规则判断，聚合源的 `explicit_reset_claim` 和 `tease_classification` 不能覆盖正文中的明确预告或完成声明。
...
| 候选 | 提到了重置，但没有可验证时间 |
| 预测 | 暗示性文字推导出的有界时间窗口 |
| 已公布 | 动态明确承诺重置；未给出时间时显示“时间待定” |
| 已确认 | 动态明确表示重置已经传播或完成 |

相对时间按 Tibo 所在的 `America/Los_Angeles` 日历解释。例如 “Soon, but not today” 只生成当地下一自然日的预测窗口，不生成具体时刻。动态中的 PT / PST / PDT 统一按当地太平洋时间解释，随日期切换夏令时；明确的时分保留为计划时刻，不补造执行区间。真实确认不能被后来的低等级解析覆盖；同一原帖的正文解析可以纠正旧版公共模型推断出的状态。

首页与详情优先显示明确预告、超时待确认和最近 24 小时的已确认重置。这些状态不显示历史模型的概率条。没有上述状态时，再显示公共接口的 24 小时概率；`signal_percent` 等承诺评分不当作统计概率。确认后的原帖时间标注为“确认时间”，不冒充实际执行时间，也不自动生成下一次重置承诺。
...
预测窗口到达不会自动变成“已确认”。明确预告过时后保留“待确认”，最多保留 7 天；只有确认动态才更新最近重置事实，并清除该确认之前的预测。公共模型的 `last_reset_at` 若没有对应的已确认记录，只作为模型输入，不单独建立确认事实。Tibo 重置与账号 5 小时/周额度重置分别显示。事实记录保存动态 ID、URL、时间、状态、规则和正文 SHA-256；另保留最近 16 条相关原文供判断依据页面查看。
